...
def group_by_centroid(restaurants, centroids):
    ...
    """
    # List comprehension is too long for one line. Bad style.
    pairs = []
...
def find_predictor(user, restaurants, feature_fn):
    ...
    s_xy = sum([(p[0]-mean_x)*(p[1]-mean_y) for p in zip(xs, ys)])
...
def best_predictor(user, restaurants, feature_fns):
    ...
    best_fn = max(feature_fns, key = lambda fn: find_predictor(user, reviewed, fn)[1])
...
def rate_all(user, restaurants, feature_fns):
    ...
    for r in restaurants:
        ...
        if r in reviewed:
            predicted_ratings[r_name] = user_rating(user, r_name)
        else:
            predicted_ratings[r_name] = predictor(r)
